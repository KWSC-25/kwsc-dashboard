...
const Dashboard = () => {
    ...
    // VIEW TOGGLING (60s Main Panel / 30s Slider)
    useEffect(() => {
        ...
        <div className="dashboard-fixed-container">
                ...
                {activeSystem === 'complaint' ? (
                                ...
                                <motion.div key="main" initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }}>
                                    <div className="main-tables-grid animate-fade-in">
                                        <div className="grid-3">
                                            <UnderperformingTable title="UNDERPERFORMING ENGINEERS WATER" typeColor="#38bdf8" iconClass="fas fa-tint" onEngineerClick={(name) => handleEngineerClick(name, 2)} typeId={2} subTypeIds="1,13,16"/>
                                            <UnderperformingTable title="UNDERPERFORMING ENGINEERS SEWERAGE" typeColor="#a78bfa" iconClass="fas fa-biohazard" onEngineerClick={(name) => handleEngineerClick(name, 1)} typeId={1} subTypeIds="21,108"/>
                                            <div className="panel" style={{ padding: '15px' }}>
                                                <TopPerformersTable title="TOP ENGINEERS WATER" onEngineerClick={(name) => handleEngineerClick(name, 2)} />
                                                <TopPerformersTable title="TOP ENGINEERS SEWERAGE" onEngineerClick={(name) => handleEngineerClick(name, 1)} />
                                            </div>
                                        </div>
                                    </div>
                                </motion.div>
                    ...
                    <div className="hydrant-content-wrapper animate-fade-in" >
                        ...
                        <div className="hmp-triple-grid">
                            <div className="hmp-grid-item"><HydrantPerformance/></div>
                            <div className="hmp-grid-item"><MobileAppGraph/></div>
                            <div className="hmp-grid-item">
                                <div className="report-content header-green coming-soon-box">
                                    <div className="section-header">
                                        <div className="section-header-green"><i className="fas fa-clock"></i> HYDRANTS OPERATIONAL HOURS</div>
                                    </div>
                                    <div className="placeholder-content"><div style={{color: 'var(--hmp-green)'}}>COMING SOON...</div></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="dashboard-grid">
                            <div className="report-content header-orange">
                                <div className='section-header' >
                                    <div className={reportView === 'aging' ? 'section-header-aging' : reportView === 'summary' ? 'section-header-orders' : 'section-header-summary'}>
                                        <i className={reportView === 'aging' ? 'fas fa-hourglass-half' : reportView === 'summary' ? 'fas fa-file-invoice' : 'fas fa-droplet'}></i> 
                                        {reportView === 'aging' ? ' HYDRANTS AGING ANALYSIS' : reportView === 'summary' ? ' HYDRANTS DAILY ORDER SUMMARY' : ' HYDRANTS DAILY GALLONS SUMMARY'}
                                    </div>
                            ...
                            <div className="report-content header-green coming-soon-box">
                                <div className="section-header">
                                    <div className="section-header-red"><i className="fas fa-shield-virus"></i> RED ZONE VIOLATIONS</div>
                                </div>
                                <div className="placeholder-content"><div style={{color: 'red'}}>COMING SOON...</div></div>
                            </div>
                        </div>
                    </div>
                )}
            ...
            {showModal && selectedEngineer && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="modal-content modern-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            <div className="stats-grid">
                                ...
                                <div className="stat-card pending"><span className="label">Pending</span><span className="value">{selectedEngineer.pending}</span></div>
                                <div className="stat-card resolved"><span className="label">Resolved</span><span className="value">{selectedEngineer.resolved}</span></div>
                                <div className="stat-card wip"><span className="label">WIP</span><span className="value">{selectedEngineer.wip}</span></div>
                            </div>
