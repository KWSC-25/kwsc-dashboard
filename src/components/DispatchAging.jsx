...
const DispatchAging = () => {
    ...
    const getCSSVar = (varName) => {
        ...
        const getPerc = (count) => {
            if (!data.chart.total_pending_ots || data.chart.total_pending_ots === 0) return '0%';
            return `${((count / data.chart.total_pending_ots) * 100).toFixed(1)}%`;
        };
    const chartConfig = {
        labels: ['< 24h', '24h-48h', '48h-72h', '> 72h'],
        datasets: [{
            data: [data.chart.less_24, data.chart.p_24_48, data.chart.p_48_72, data.chart.above_72],
                ...
                <table>
                    ...
                    <tbody>
                        {data.table.map((row, i) => {
                            ...
                            const isSevere = row.pending_above_72h > 0;
                            
                            return (
                                <tr key={i} className={isSevere ? 'blink-row' : ''}>
                                    <td className="hydrant-name-cell">{row.hydrant_name}</td>
                                    <td className="val-24w">{row.pending_less_than_24h}</td>
                                    <td className="val-24">{row.pending_24h_to_48h}</td>
                                    <td className="val-48">{row.pending_48h_to_72h}</td>
                                    <td className="val-72">{row.pending_above_72h}</td>
                                </tr>
                            );
                        })}
                    </tbody>
            ...
            <div className="aging-chart-section">
                <div className="chart-wrapper">
                    ...
                    <div className="chart-center-text">
                        <span className="total-num-aging">{data.chart.total_pending_ots}</span>
...
<div>
                    ...
                    <div>
                        ...
                        &gt; 24 Hrs <span className="perc-tag-2">({getPerc(data.chart.p_24_48)})</span>
                    </div>
                    <div>
                        <span className="dot-a orange"></span> 
                        &gt; 48 Hrs <span className="perc-tag-3">({getPerc(data.chart.p_48_72)})</span>
                    </div>
                    <div>
