...
const UnderperformingTable = ({ data, idleData, title, typeColor, iconClass }) => {
  const maxRate = data?.length > 0 ? Math.max(...data.map(e => parseFloat(e.pending_rate))) : 0;
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Pen/Total</th>
            <th>Rate</th>
            <th style={{ color: '#f87171' }}>Delay</th>
          </tr>
        </thead>
        <tbody>
          {data?.map((eng, idx) => (
            <tr key={idx} className={parseFloat(eng.pending_rate) === maxRate ? "row-alarm" : ""}>
              <td style={{ lineHeight: '1.2' }}>
                <span style={{ fontWeight: 'bold' }}>{eng.xen_name}</span><br />
                <small style={{ color: '#94a3b8', fontSize: '0.7rem' }}>{eng.town_name}</small>
              </td>
              <td>{eng.pending_count}/{eng.total_count}</td>
              <td style={{ color: parseFloat(eng.pending_rate) > 40 ? '#f87171' : 'inherit' }}>
                {eng.pending_rate}%
              </td>
      ...
      <div className="idle-card">
        ...
        {idleData?.map((item, idx) => (
          <div key={idx} className="idle-row-data" style={{ display: 'flex', gap: '5px', fontSize: '0.65rem' }}>
            {/* Logic: Strip 'COMPLAINT-' and only show the number */}
            <span style={{ flex: '0.8' }}>{item.complaint_no?.split('-')[1] || item.complaint_no}</span>
            
            <span style={{ flex: '1.5', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {item.type}
            </span>
            ...
            <span style={{ flex: '0.7', color: '#f87171', fontWeight: 'bold', textAlign: 'right' }}>
              {item.overdue_hrs}h
            </span>
          </div>
        ))}
