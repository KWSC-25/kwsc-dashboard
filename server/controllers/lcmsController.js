...
export const importLcmsExcel = async (req, res) => {
    try {
        ...
        const workbook = xlsx.read(req.file.buffer, { type: 'buffer' }); // Removed cellDates: true
        const sheetName = workbook.SheetNames[0]; 
        const worksheet = workbook.Sheets[sheetName];

        // Use raw: false and specify the Date Next Format (dateNF)
        // This treats the date as a string in the format Postgres expects (YYYY-MM-DD)
        ...
        for (const rawRow of rawData) {
            ...
            // No more complex JS Date logic needed. 
            // row['next_date'] will now be a string like "2026-03-30"
            const values = [
                row['court_name'] || null,
                row['case_title'] || null,
                row['next_date'] || null, 
