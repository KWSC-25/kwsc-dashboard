    ...
    try {
        ...
        LEFT JOIN (
            ...
            SELECT 
                ...
                SUM(CASE WHEN ots.api_created_at >= CURDATE() AND ots.status IN ('pending', 'pending_alignment') THEN 1 ELSE 0 END) AS pending_today,
                ...
                SUM(CASE WHEN ots.api_created_at >= DATE_FORMAT(CURDATE(), '%Y-%m-01') AND ots.status IN ('pending', 'pending_alignment') THEN 1 ELSE 0 END) AS pending_month
            FROM ots_order ots
